...
def check_time(time):
    if not isinstance(time, Real):  # 判断是不是数字
...
def check_msg_dict(msg_dict):
    spec = SPEC_BY_TYPE.get(msg_dict['type'])
    if spec is None:
        raise ValueError('unknown message type {!r}'.format(msg_dict['type']))

    for name, value in msg_dict.items():
